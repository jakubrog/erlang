...
%% API
-export([createMonitor/0, addStation/3, addValue/5, test/0, removeValue/4, getOneValue/4, getStationMean/3, getDailyMean/3, getMaximumGradientStation/2]).
...
addValue(Name, Time, Type, Result, P) ->
  ...
  [Station | _] =  [X || X <- Stations, maps:get(name, X) == Name],% or maps:get(name, X) == Name],
...
removeValue(Name, Time, Type, P) ->
  ...
  [ToDelete | _] = [X || X<-Results, maps:get(time, X) == Time],%Add rest
...
getStationMean(Name, Type, P) ->
  ...
  [Station | _] =  [X || X <- Stations, maps:get(name, X) == Name],
  Results = maps:get(results,Station),
  Result = [X || X<-Results, maps:get(type, X) == Type],%Add rest
